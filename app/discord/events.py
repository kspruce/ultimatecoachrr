...
def register_event_listeners(app):
    ...
    # Use a flag to track if listeners have been registered
    if hasattr(register_event_listeners, 'registered'):
        return
    register_event_listeners.registered = True
    
    try:
        # Session events
        from app.models.session import SessionPlan
        
        @event.listens_for(SessionPlan, 'after_insert')
