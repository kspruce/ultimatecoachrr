...
class Point(db.Model):
    ...
    our_line_type = db.Column(db.String(20), nullable=False)  # O-line, D-line
    ...
    starting_position = db.Column(db.String(20), nullable=False)  # offense, defense
    point_outcome = db.Column(db.String(20), nullable=False)  # scored, conceded
    duration = db.Column(db.Integer, nullable=True)  # in seconds
    timestamp_in_video = db.Column(db.Integer, nullable=True)  # in seconds
    ...
    @property
    def is_break(self):
        """Return True if this was a break point (we scored on defense or they scored on offense)."""
        return (self.our_line_type == 'D-line' and self.we_scored) or \
               (self.our_line_type == 'O-line' and self.they_scored)
    
    @property
    def is_hold(self):
        """Return True if this was a hold point (we scored on offense or they scored on defense)."""
