...
def edit(team_id):
    ...
    if form.validate_on_submit():
        ...
        # Logo is a URL — save it directly (empty string → None to clear)
        logo_url = form.logo.data.strip() if form.logo.data else None
        team.logo = logo_url if logo_url else None
        db.session.commit()
